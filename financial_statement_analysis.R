# R Programming: assignment - 2
# Scenario: 
# You are a Data Scientist working for a consulting firm. One of your
# colleagues from the Auditing department has asked you to help them assess the
# financial statement of organization X.
# You have been supplied with two vectors of data: monthly revenue and monthly
# expenses for the financial year in question. Your task is to calculate the following
# financial metrics:
# - profit for each month
# - profit after tax for each month (the tax rate is 30%)
# - profit margin for each month - equals to profit a after tax divided by revenue
# - good months - where the profit after tax was greater than the mean for the year
# - bad months - where the profit after tax was less than the mean for the year
# - the best month - where the profit after tax was max for the year
# - the worst month - where the profit after tax was min for the year

# All results need to be presented as vectors.
# Results for dollar values need to be calculated with $0.01 precision, but need to be
# presented in Units of $1,000 (i.e. 1k) with no decimal points.
# Results for the profit margin ratio need to be presented in units of % with no
# decimal points.
# Note: You colleague has warned you that it is okay for tax for any given month to be
# negative (in accounting terms, negative tax translates into a deferred tax asset).

#Data
revenue <- c(14574.49, 7606.46, 8611.41, 9175.41, 8058.65, 8105.44, 11496.28, 9766.09, 10305.32, 14379.96, 10713.97, 15433.50)
expenses <- c(12051.82, 5695.07, 12319.20, 12089.72, 8658.57, 840.20, 3285.73, 5821.12, 6976.93, 16618.61, 10054.37, 3803.96)

# Solutions
# Profit = revenue - expenses
profit <- revenue - expenses

# Tax is 30% Of Profit (Precision - 2 decimal places)
tax <- round(0.30 * profit, 2)

# Profit after tax deduction
profit_after_tax <- profit - tax

# Profit Margin profit after tax Over Revenue (percentage precision)
profit_margin <- round(profit_after_tax / revenue, 2) * 100

# Mean Profit After Tax For The 12 Months
mean_profit <- mean(profit_after_tax)

# Good months - where the profit after tax was greater than the mean for the year
good_months <- profit_after_tax > mean_profit

# Bad Months Are The Opposite Of Good Months !
bad_months <- profit_after_tax < mean_profit # alternative: use !good_months

# The Best Month Is Where Profit After Tax Was Equal To The Maximum
best_month <- profit_after_tax == max(profit_after_tax)

#The Worst Month Is Where Profit After Tax Was Equal To The Minimum
worst_month <- profit_after_tax == min(profit_after_tax) 

# Convert All Calculations To Units Of One Thousand Dollars
final_revenue <- round(revenue / 1000)
final_expenses <- round(expenses / 1000)
final_profit <- round(profit / 1000)
final_profit_after_tax <- round(profit_after_tax / 1000)

#Print Results
final_revenue
final_expenses
final_profit
final_profit_after_tax
profit_margin
good_months
bad_months
best_month
worst_month

# Alternative: Matrix representation
final_output <- rbind(
  final_revenue,
  final_expenses,
  final_profit,
  final_profit_after_tax,
  profit_margin,
  good_months,
  bad_months,
  best_month,
  worst_month
)
final_output
