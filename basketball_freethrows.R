...
# You have been supplied data for in-game statistics of NBA:
# - FieldGoalAttempts
# - FieldGoals
# - Games
# - MinutesPlayed
...
free_throw <- rbind(KobeBryant_FT, JoeJohnson_FT, LeBronJames_FT, CarmeloAnthony_FT, DwightHoward_FT, ChrisBosh_FT, ChrisPaul_FT, KevinDurant_FT, DerrickRose_FT, DwayneWade_FT)
# renaming rows and columns
rownames(free_throw) <- Players
colnames(free_throw) <- Seasons
# removing the vectors to save space
...
# plotting function
myplot <- function(data, ppl=1:10) {
  matplot(t(data[ppl,,drop=F]), type="b", pch=15:18, col=c(1:4,6), main="Basketball Players Analysis")
  legend("bottomleft", inset=0.01, legend=Players[ppl], col=c(1:4,6), pch=15:18, horiz=F)
}
...
myplot(fta/Games)
# Insights:
# Chris Paul gets few attempts per game

#Part 2 - Free Throw Accuracy
myplot(free_throw / fta) 
# Insights:
# Chris Paul's accuracy is one of the highest. 
# Dwight Howard's FT Accuracy is extremely poor compared to other players.

#Part 3 - Player Style Patterns Excluding Free Throws
myplot((Points - free_throw) / FieldGoals)
#Insights:
#1. Every players preference for 2 or 3 point shots
#   changes throughout their career. Most drastic change is seen in Joe Johnson.
#2. There is one exception. One player always scores a 2-pointer. It's none other than Dwight Howard! 
#   That's cause he can be close to the basket or even in contact with it. Free throws, on the other hand require
#   the player to stand 15ft (4.57m) away from the hoop. That's probably why Dwight Howard's Free Throw Accuracy is poor.